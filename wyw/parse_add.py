    ...
    for i in range(i, len(text)):
        ...
        if arr.__len__() != 0:
            for key in arr:
                ...
                if isomit:
                    break
        if arr.__len__()==0 or i == len(text)-1:
            have = sorted(have.items(), key=lambda item: item[1], reverse=True)
            for k in have:
                l = len(nameDic[k[0]])
                if l == 10 and res["province"] == "":
            ...
            if res["province"] != "" and res["city"] != "" and res["district"] != "":
                break
            if j - i == 0:
                acSearch(text, i + 1, res, pos)
            else:
                acSearch(text, i, res, pos)
            return
#地址解析
def do_parse(text):
    street=""
...
if __name__ == '__main__':
    print(parse_address("河南省三门峡市灵宝市焦村镇滑底村委会"))
